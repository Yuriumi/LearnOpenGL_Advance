...
## 面剔除 [教程地址](https://learnopengl-cn.github.io/04%20Advanced%20OpenGL/04%20Face%20culling/)

绘制立方体时我们可以想象,我们最多可**同时**看见立方体的三个面,那么对其他的面的渲染是**多余**的;并且立方体的内部我们是不关心的,也看不到的,如果不去渲染这些面,我们将省去很大的开销!

我们只渲染面向观察者的面,如何绘制只面向观察者的面正式面剔除(face culling)所做的!

OpenGL能够检查所有面向(front facing)观察者的面,并渲染他们,而丢弃那些背向的面,节省很多片段着色器的调用(它们开销很大).但我们需要告诉OpenGL那些面是正向面,而那些不是.

### 环绕顺序

OpenGL使用的技巧是顶点数据的环绕顺序;当我们定义三角形顶点时,可能是逆时针的,可能是顺时针的;我们需要为这三个顶点设置环绕顺序.

![三角面的环绕顺序](https://learnopengl-cn.github.io/img/04/04/faceculling_windingorder.png)
...
在渲染图元的过程中,这些信息将决定这个三角形是背对的还是正对的.

![图元渲染](https://learnopengl-cn.github.io/img/04/04/faceculling_frontback.png)

### 使用面剔除

面剔除默认是禁用的.开启他的方法与之前的深度测试等等的开启方式是相同的;开启后我们就可以使用面剔除选项的了.

`glEnable(CL_CULL_FACE);`

但是,上一节的草出现了问题(地面和窗户也是),内面全部被丢弃了,如果我们想渲染草这样的非封闭物体,我们就必须关闭面剔除.

OpenGL允许我们改变需要剔除的面的类型.如果我们想剔除的是正面而不是背面我们可以通过调用`glCullFace(GL_FRONT);`来定义这一行为.
